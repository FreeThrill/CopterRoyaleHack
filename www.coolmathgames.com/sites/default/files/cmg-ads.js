var version = "1.6-rewardads-v0.67";
...
function adBreakRequest() {
  if (window.parent && window.parent.document) {
    ...
    if (window.parent.document.body.classList.contains('subscribed-users')) {
      //console.log('cmg Subscribed user skipping the rest of the script.');
...
function isMobileDevice() {
  //console.log('is mobile device Function called');
  var hasMobileClass = /Android|iPhone/i.test(navigator.userAgent);
...
function sendResizeMessage(width, height) {
  // If width and height are not provided, set them based on isMobile
...
function resetToOriginalSize() {
  // console.log(
  //   "cmg Resetting to original size:",
  //   originalWidth,
  //   "x",
  //   originalHeight
  // );
...
function cmgAdBreak() {
  ...
  if (
    ...
    handleAdBreakComplete();
    console.log(
      "cmgAdBreak:Too early to invoke midroll ads, wait for " +
        adBreakInterval +
        " milliseconds"
    );
    return;
...
function adBreakSpecificLogic() {
  ...
  if (!cmgAInvD) {
    ...
    // console.log(
    //   new Date() +
    //     " CMGAds Midroll Ads inside game: window.PW_CMG.startAuction...." +
    //     gameURL
    // );
...
function adRewardsBreakSpecificLogic() {
  ...
  justAMomentLoaderTimer = setTimeout(() => {
    if (typeof is_ad_loaded != "undefined" && !is_ad_started) {
      ...
      console.log(
        "cmgAdBreak Ad is loaded in 8 seconds so not removing the justamomnetloader/ad container..."
      );
    }
  }, 8000);
  ...
  if (!cmgAInvD) {
    ...
    // console.log(
    //   new Date() +
    //     " CMGAds Rewarded Ads inside game: window.PW_CMG.startAuction...." +
    //     gameURL
    // );
...
function setupAuctionCallback() {
  ...
  if (!cmgAInvD) {
    ...
    console.log(
      new Date() +
        " cmgRewardsAdBreak Midroll inside game: window.PW_CMG.startAuction...." +
        gameURL
    );
  ...
  $(".playNow")
    ...
    .on("click", function (e) {
      ...
      if (adsManager) {
        ...
        $(".ad-container").show();
       // console.log("cmgAd remaining time: " + adsManager.getRemainingTime());
        ...
        if (
          ...
          var nonLinearAdTimer = setTimeout(() => {
            console.log(
              "cmgAdBreak Ad is non-linear so destroying in 5 seconds ..."
            );
            removeMidrollAndReinit(); //remove  midroll or rewarded ads
          }, 5000);
        }
...
Ads.prototype.onAdEvent_ = function (adEvent) {
  ...
  if (adEvent.type === google.ima.AdEvent.Type.STARTED) {
    ...
    // console.log(
    //   "cmg Ad completed or user closed. Preparing to resize back to original dimensions: " +
    //     originalWidth +
    //     "x" +
    //     originalHeight +
    //     "."
  ...
  switch (adEvent.type) {
    case google.ima.AdEvent.Type.LOADED:
      console.log(
        "cmgAdBreak AdEvent type Loaded fired - starting 8 second timer to kill the Ad if it is not started"
      );
      is_ad_loaded = true;
      clearTimeout(justAMomentLoaderTimer);
      ...
      adLoadedTimer = setTimeout(() => {
        if (typeof is_ad_started != "undefined" && !is_ad_started) {
          console.log(
            "cmgAdBreak Ad did not start in 8 seconds so removing it..."
          );
          removeMidrollAndReinit(); //remove  midroll or rewarded ads
        }
      }, 8000);
    ...
    case google.ima.AdEvent.Type.STARTED:
      console.log(
        "cmgAdBreak AdEvent type Started fired - clearing Loader Timer"
      );
      is_ad_started = true;
